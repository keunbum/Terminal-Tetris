...
3) Ghost Piece(켜진 경우)가 북쪽을 향한 채 아래에 나타난다. 기존 블록이 Tetrimino의 경로에 있는 경우 Tetrimino는 한 행을 즉시 떨어지지 않지만, 생성된 Tetrimino의 몇 픽셀이 표시되어(하드웨어가 허용하는 경우) 플레이어가 Skyline 위에서 블록을 조작하는 데 도움이 된다.
...
테트로미노가 표면에 처음 착지하면 잠기기([Lock Down](#lock-down-플레이-중인매트릭스에-나와-있는-테트로미노가-잠겨-플레이어가-이를-더-이상-조작할-수-없는-지점)) 전에 
시간 제한이 있다.  
...
테트로미노의 각 [Mino](#mino-테트로미노를-이루는-단일-정사각형-모양의-구성-요소이다)는 이동 완료 시 적절한 셀 위치로 "스냅"
(아마 사진이나 스크린샷 찍듯이 이펙트가 나타나는 걸 말하는 듯.) 되지만, 중간에 일어나는 테트로미노 이동은 부드럽게 나타난다.  
오른쪽, 왼쪽 및 아래쪽 이동만 허용된다. [점유 중인 셀](#block-매트릭스의-셀에-잠긴lock-down-단일-블록-기존-블록이라고도-한다)과 매트릭스 벽 및 바닥으로의 이동은 허용되지 않는다.
...
자동 반복은 이동 버튼을 누르고 있는 동안 다음 테트로미노가 나오기 전(현재 테트로미노 [잠금](#lock-down-플레이-중인매트릭스에-나와-있는-테트로미노가-잠겨-플레이어가-이를-더-이상-조작할-수-없는-지점) 후)까지 계속된다.
...
하드 드롭 명령은 테트로미노를 즉시 떨어뜨리고 바로 아래 표면에 고정시킨다. 하드 드롭된 테트로미노가 현재 위치에서 [잠기기](#lock-down-플레이-중인매트릭스에-나와-있는-테트로미노가-잠겨-플레이어가-이를-더-이상-조작할-수-없는-지점)까지 떨어지는 데 걸리는 시간은 0.0001초이다. 하드 드롭에는 자동 반복이 없다.