...
이 게임에서 화면에 물체를 그리는 기본 원리는 [화가 알고리즘](https://ko.wikipedia.org/wiki/%ED%99%94%EA%B0%80_%EC%95%8C%EA%B3%A0%EB%A6%AC%EC%A6%98)이다.  
마치 그림을 그릴 때의 방식처럼 가장 먼 배경부터 시작하여 가까워지는 순으로 물체를 그려나가는 전략을 일컫는다.
...
대략 넉넉잡아 40 x 40 정도 크기의 배열이 필요하다고 가정해보자.  
60FPS라 가정하면 초당 60 x 40 x 40 = 96,000개의 문자를 터미널에 출력해야 한다.  
