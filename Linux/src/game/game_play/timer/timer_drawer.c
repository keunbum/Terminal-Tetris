...
static void wdraw_timer_frame_at(int pos_x_wprint, int pos_y_wprint)
{
    ...
    wdraw_rows_newline_at_r(S_STR_NUM, s_strs, TIMER_FRAME_WIDTH, pos_x_wprint, pos_y_wprint);
}
...
void* run_timer_drawer_with(void* arg)
{
    ...
    timer_drawer_t* timer_drawer_ptr = (timer_drawer_t*)arg;
    realtime_timer_t* timer_ptr = &timer_drawer_ptr->timer;

    /* Set sigset */
    sigemptyset(&timer_ptr->sigset);
    sigaddset(&timer_ptr->sigset, timer_ptr->timer_sig);

    /* Create timer */
    sigevent_t sev;
    sev.sigev_notify = SIGEV_SIGNAL;
    sev.sigev_signo = timer_ptr->timer_sig;
    sev.sigev_value.sival_int = 0;
    sev._sigev_un._sigev_thread._function = NULL;
    if (timer_create(timer_ptr->clockid, &sev, &timer_ptr->timerid) == -1) {
        handle_error("timer_create() error");
    }

    /* Start timer */
    if (timer_settime(timer_ptr->timerid, 0, &timer_ptr->its, NULL) == -1) {
        handle_error("timer_settime() error");
    }

    /* Run main logic */
    set_realtime_timer(timer_ptr, true);
    doit_drawer_main_logic(timer_ptr, &timer_drawer_ptr->draw_module);

    /* Delete timer */
    if (timer_delete(timer_ptr->timerid) == -1) {
