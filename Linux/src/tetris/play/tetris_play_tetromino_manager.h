...
#define PIECE_SPEED_DRAW_POS_X_WPRINT (3)
#define PIECE_SPEED_DRAW_POS_Y_WPRINT (50)
...
typedef struct {
    pos_int_t piece_queue_pos_wprint;
    ...
    fixed_queue_t piece_que;
    frame_t piece_queue_frame;
    frame_t hold_frame;
} tetromino_manager_t;
...
static inline void wdraw_piece_speed(const tetromino_manager_t* man)
{
    wprintf_at_r(PIECE_SPEED_DRAW_POS_X_WPRINT, PIECE_SPEED_DRAW_POS_Y_WPRINT, L"SPEED: %-4.2lf", man->piece_velocity);
}

static inline void update_tetromino_manager_info(tetromino_manager_t* const out_man, int lines)
{
    out_man->is_swaped_once = false;
    out_man->piece_velocity += lines * out_man->unit_velocity;
    wdraw_piece_speed(out_man);
}
