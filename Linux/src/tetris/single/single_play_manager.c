...
static game_play_manager_module_arg_t g_s_play_manager = {
    ...
    .board = {
        .height = GAME_PLAY_BOARD_HEIGHT,
        .width = GAME_PLAY_BOARD_WIDTH,
        .height_wprint = GAME_PLAY_BOARD_HEIGHT_WPRINT,
        .width_wprint = GAME_PLAY_BOARD_WIDTH_WPRINT,
    },
    .timer_drawer = {
        .timer = {
...
static void load_ui(void)
{
    ...
    int res = load_game_play_scene(g_s_play_manager.mode, g_s_play_manager.screen_start_pos_x_wprint, g_s_play_manager.screen_start_pos_y_wprint);
    if (res == -1) {
        handle_error("load_game_play_scene() error");
    }
}

static int ready_getset_go(void)
{
    debug();
    ...
    for (int cur_sec = g_s_play_manager.ready_getset_go_sec; cur_sec >= 0; --cur_sec) {
        ...
        const pos_t pos_y_in_wprint = GAME_PLAY_BOARD_FRAME_START_POS_Y_WPRINT + GAME_PLAY_BOARD_FRAME_WIDTH - 2;
        if (cur_sec == 0) {
...
static void* main_func_game_main_module(void* arg)
{
    ...
    while (!is_game_over) {
        ...
        while (true) {
            ...
            int status = move_a_tetromino(board, &tetromino_obj);
            draw_a_tetromino_r(&tetromino_obj);
...
void* run_single_play_mode(void* arg)
{
    ...
    while (true) {
        ewprintf("run_single_play_mode() HI\n");
